	...
	if ing.Spec.Backend != nil {
		...
		if altUps == nil {
			...
			for _, loc := range servers[defServerName].Locations {
				...
				if altEqualsPri {
					klog.Warningf("alternative upstream %s for location %s/%s in Ingress %s/%s is primary upstream in Other Ingress!",
						altUps.Name, servers[defServerName].Hostname, loc.Path, ing.Namespace, ing.Name)
					break
				}
	...
	for _, rule := range ing.Spec.Rules {
		for _, path := range rule.HTTP.Paths {
			...
			for _, loc := range server.Locations {
				...
				if altEqualsPri {
					klog.Warningf("alternative upstream %s for location %s/%s in Ingress %s/%s is primary upstream in Other Ingress!",
						altUps.Name, server.Hostname, loc.Path, ing.Namespace, ing.Name)
					break
				}
